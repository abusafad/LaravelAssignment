...
For any questions regarding the exam please send inquiry to <a href="mailto:mustafa.qadoum@netronsolutions.com">mustafa.qadoum@netronsolutions.com</a> or whatsapp at: 00962798996721.
...
1. Initiate a Laravel project using version 9 or higher.
2. Integrate the default login functionality utilizing the [laravel/ui](https://packagist.org/packages/laravel/ui) package.
3. Create a page to list all users (`users.index`) within a table.
4. Develop a page to display information about a single user (`users.show`).
5. Design a page to present a form for creating a new user (`users.create`).
6. Implement a page for editing user details (`users.edit` / `users.update`).
7. Include a button to delete a user (`users.destroy`).
8. Incorporate a page to list all soft deleted users (`users.trashed`).
9. Integrate a button to restore a soft deleted user (`users.restore`).
...
- Ensure that all routes for users are protected by the `auth` middleware.
- The `prefixname` variable should be limited to one of the following options: `[Mr, Mrs, Ms]`.
- When working with forms, include the `enctype=multipart/form-data` attribute to enable the uploading of user photos.
<br>
...
<p align="center">
<small>Prepared for NetronSolutiots  Applicants only.<br>Do not reproduce document elsewhere.</small>
</p>
